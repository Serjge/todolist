...
import {TASK_ACTIONS_TYPE} from "../action/taskAction";
...
test('add todolist task', ()=>{
    const addTodolistTask = TasksReducer(tasks,{type: TODOLIST_ACTIONS_TYPE.TL_ADD, payload:{ todolistId: '2' , title: '111'}})
    expect(addTodolistTask['2'].length).toBe( 0)
})

test('remove tasks', ()=>{
    const removeTasks = TasksReducer(tasks,{type:TODOLIST_ACTIONS_TYPE.TL_REMOVE, payload:{ todolistId: '1'}})
